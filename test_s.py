...
def test_listify_str():
    assert type(S("a").scope_req) is list

def test_listify_list():
    assert type(S(["a","b"]).scope_req) is list
...
def test_1_or_ok():
    ...
    assert s_comb.result_in(token_scopes) == True
...
def test_1_or_ko():
    ...
    s_comb = s_a | s_b
    logger.debug(s_comb.history)
    assert s_comb.result_in(token_scopes) == False
